...
function main() {
    if (app.documents.length === 0) { alert("Ошибка: Откройте шаблон ОБЛОЖКИ!"); return; }
    ...
    for (var i = 0; i < childFolders.length; i++) {
        ...
        try {
            ...
            if (printChoice === "Photofier") {
                ...
                while (bookNum.length < 3) bookNum = "0" + bookNum; // Делаем 001, 002...
                fileName = "00-" + bookNum; // YY=00 (Обложка), XXX=Номер книги
            } else {
